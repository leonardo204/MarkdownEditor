...
이미지가 많은 대용량 문서(9,000줄 · 이미지 56장 · 2.5MB)에서 로딩과 스크롤이 느리다는 고객 요구로 시작한 성능 작업.

- perf: 프리뷰 로컬 이미지를 base64 인라인 → `me-asset://` 커스텀 스킴 온디맨드 서빙으로 전환
  - HTML 크기 3.7MB → 0.45MB, WKWebView 이미지 캐시 활용
  - `loading="lazy" decoding="async"` + 실제 픽셀 크기(`width`/`height`) 주입으로 레이지 로드 시 레이아웃 시프트 제거 (EXIF 회전 반영)
  - 이미지 헤더 읽기 결과를 (경로, mtime, 크기) 키로 캐시
- perf: CDN 웹 리소스(highlight.js·KaTeX·Mermaid·pako)를 번들 서빙으로 전환 — 콜드 로드 1.7초 → 0.2초대
  - `katex.min.css`만 폰트 미번들로 CDN 유지, `media="print" onload` 논블로킹 로드
- perf: 에디터 스크롤 시 문서 전체 레이아웃이 무효화되던 문제 수정
  - `applyTheme`가 매 스크롤 틱마다 동일한 폰트·색을 재대입해 전체 레이아웃을 무효화 (스크롤 위치에 비례해 틱당 250ms~2초)
  - 마지막 적용값 캐시로 변경 시에만 대입 (포커스 모드 딤이 게터를 오염시키므로 게터 비교는 사용 불가)
- perf: 대용량 문서 레이아웃 사전 계산 — 로드 후 8,000자 청크로 전체 글리프 생성
  - 프리뷰 스크롤이 에디터를 임의 위치로 점프시킬 때의 글리프 생성 블로킹 589ms → 0.8ms
  - 프리뷰 켠 상태 연속 스크롤 틱 간격 ~600ms → 16ms
- perf: 스크롤 틱당 반복 비용 제거 — 259KB 문자열 전체 비교, HTML 해시, O(n) 라인 계산(→ 이진 탐색), SwiftUI 전체 재평가
- fix: 에디터 ↔ 프리뷰 스크롤 동기화 에코 루프 제거
  - 동기화가 만든 스크롤이 사용자 스크롤로 오인되어 반대편을 되밀던 문제 (프리뷰 켠 상태에서 스크롤 이벤트 82% 유실)
  - 프리뷰→에디터는 사용자 입력 게이트, 에디터→프리뷰는 구간 플래그로 차단 (벽시계 타이머 방식 폐기)
  - 파일을 열면 에디터가 문서 78% 지점으로 끌려가던 증상도 함께 해소
- fix: 드래그 드롭·열기 패널·최근 파일로 연 문서에서 폴더 접근 권한이 복원되지 않아 이미지가 표시되지 않던 문제
  - `DocumentManager.loadFile`에서 security-scoped bookmark 개시, 디렉토리당 1회만 클레임
  - 경로를 해석하지 못한 이미지는 시도한 경로를 보여주는 placeholder로 치환 (`file://` 폴백은 `loadHTMLString` 페이지에서 렌더링 불가)
- fix: 프리뷰 CSP에 `frame-src`/`object-src` 명시 — SVG 내부 스크립트가 동일 출처에서 실행될 경로 차단
